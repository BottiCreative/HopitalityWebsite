...
?>
<?php  if ($options['show_products'] || ($options['show_products'] && $_REQUEST['search'] == '1')) { ?>
	<?php 
		...
		<tr>
			...
			?>				
				...
				if (count($sort_columns)>0) { ?>
					...
					<div class="product-list-sort-header"><?php echo  t('Sort by:'); ?> 
						<select class="product-list-sort-select">
							<?php 
							$current_col = $_REQUEST['ccm_order_by_b'. $b->bID];
							foreach ($sort_columns as $col => $name) {
								$selected = ($current_col == $col && $_REQUEST['ccm_order_dir_b'. $b->bID] == 'asc') ? "selected" : "";
								echo '<option '.$selected.' value="';
								$productList->getSortByURL($col, 'asc', $bu);
								echo '">'.$name. ' ' . t('Ascending') . '</option>';
								$selected = ($current_col == $col && $_REQUEST['ccm_order_dir_b'. $b->bID] == 'desc') ? "selected" : "";
								echo '<option '.$selected.' value="';
	...
	<?php 
		...
		<p><?php echo  t('No products found'); ?></p>
	<?php  } ?>
<?php  }
