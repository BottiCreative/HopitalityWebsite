...
//ok, lets include a set name we WANT to include in this list.
$includedSetName = 'Homepage';
...
//get the product set we want to INCLUDE for this list.
$productSetToIncludeInList = null;
foreach($productSets as $productSet)
{
	...
	if($productSet->getProductSetName() == $includedSetName)
	{
		$productSetToIncludeInList = $productSet;
	}
...
<!-- begin products -->
		...
		for ($i = 0; $i < count($products); $i++) {
			...
			//check to see if the current product is NOT contained in the selected Product Set AND is part of the product set we want to include.
			if(!$selectedProductSet->Contains($pr) && $productSetToIncludeInList->Contains($pr) )
			{
