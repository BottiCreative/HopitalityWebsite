...
class ProblogDateArchiveBlockController extends BlockController {
	...
	*/
	...
	public function view() {
		...
		$query = "SELECT MIN(cv.cvDatePublic) as firstPost 
			...
			WHERE pt.ctHandle LIKE '%".$this->page_type."%' and cv.cvIsApproved = 1";		
	...
	public function check_date($date){
		$query = "SELECT cv.cvDatePublic 
			FROM CollectionVersions cv 
			INNER JOIN PageTypes pt ON cv.ctID = pt.ctID
			WHERE pt.ctHandle LIKE '%".$this->page_type."%' and cv.cvIsApproved = 1 and DATE_FORMAT(cv.cvDatePublic,'%Y-%m') = ?";		
