...
class ExpressiveDate extends DateTime {
	 ...
	 * @return string
	 */
	public function getDifferenceInWeeks($compare = null)
	{
		if ( ! $compare)
		{
			$compare = new ExpressiveDate(null, $this->getTimezone());
		}
		$diffInDays = $this->getDifferenceInDays($compare);
	 ...
	 */
	public function differenceAsWords($compare = null, $from = '', $to = '', $prefix = 'about', $suffix = '')
	{
		if ( ! $compare)
		{
			$compare = new ExpressiveDate(null, $this->getTimezone());
		...
		$diff_in_seconds = $this->getDifferenceInSeconds($compare);
		// return  $diff[0][0];
			if ($diff_in_seconds < 60) { //less than a minute
				...
				foreach ($diff as $index => $value) {
					...
					$and = $index=='s' ? 'and ' : '';
					if ($index=='d' && $value>6) {
						$words[] = $this->_stringify($this->keys['w'], floor($value/7));
						$words[] = $this->_stringify($this->keys[$index], ($value%7));
					} else {
						$words[] = $and . $this->_stringify($this->keys[$index], $value);
					}
