...
class CoreCommerceShippingType extends Object {
	...
	protected function loadController() {
      ...
      if (file_exists($file)) {
         ...
         Log::addEntry(t('The controller file for  shipping type with the handle "%s" was not found.'.PHP_EOL.'To correct this, there are two options.'.PHP_EOL.'1. Put the controller file back in models/shipping/types/%s/ to restore the shipping type.'.PHP_EOL.'2. Delete the database row from CoreCommerceShippingTypes with the shippingTypeHandle "%s" to completely remove the shipping type.'.PHP_EOL.'NOTE: If you installed this shipping type from a package, DO NOT try reinstalling it to fix this issue. First delete the row, then re-install the package.',$this->shippingTypeHandle,$this->shippingTypeHandle,$this->shippingTypeHandle),t("eCommerce Error"));
            throw new Exception(t('Missing controller file for a shipping type.  See Logs for more information'));
      }
	}
