<?php 
	...
	class CoreCommerceCheckoutStepHelper {
		...
		public function testCurrentStep() {
			if ($this->cPath == '/checkout') {
				$nts = $this->getNextCheckoutStep();
				if (is_object($nts)) {
					Controller::redirect($nts->getRedirectURL());
				}
			} else {
				$steps = $this->getSteps();
		...
		public function getCheckoutNextStepButton($text=false) {
			...
			if ($obj instanceof CoreCommerceCheckoutStep) {
				return Loader::helper('form')->submit('submit_next', ($text!==false?$text:t('Next')), array('class' => 'ccm-core-commerce-checkout-button-next'));
			}
		}
	...
	class CoreCommerceCheckoutStep {
		...
		public function getSubmitURL($json=0) {return $this->getBase() . View::url($this->path, 'submit', $json);}
