...
class CoreCommercePaymentController extends Controller {
	...
	protected function setEmailData($mh, $order, $data=null) {
		...
		foreach($order->getProducts() as $product) {
			$attributes = array();
			foreach($product->getProductConfigurableAttributes() as $ak) {
				$attributes[$ak->getAttributeKeyName()] = $product->getAttribute($ak);
			}
			...
			$products[] = array(
					...
					'price'      => $product->getProductCartDisplayPrice(),
		...
		$adjustments = array();
		foreach($order->getOrderLineItems() as $item) {
			$adjustments[] = array(
				'name'  => $item->getLineItemName(),
				'type'  => $item->getLineItemType(),
				'total' => $item->getLineItemDisplayTotal()
			);
		}
