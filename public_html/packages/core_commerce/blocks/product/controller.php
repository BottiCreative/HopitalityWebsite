...
class ProductBlockController extends BlockController {
		...
		protected function loadPropertyDisplayOrder() {
			if ($this->displayPropertyOrder != '') {
				$propertyOrder = @unserialize($this->displayPropertyOrder);
		...
		public function getSearchableContent() {
			...
			if ($this->product) {
				Loader::model('attribute/categories/core_commerce_product', 'core_commerce');
				$searchable = array($this->product->getProductName(),strip_tags($this->product->getProductDescription()));
				foreach(CoreCommerceProductAttributeKey::getList() as $property) {
					if ($property->akIsSearchable && $property->atHandle == 'text') {
						$dak = CoreCommerceProductAttributeKey::getByID($property->akID);
						$av = $this->product->getAttributeValueObject($dak);
						if (is_object($av)) {
							$searchable[] = $av->getValue('display');
						}
					}
				}
		...
		*/
		...
		public function save($data) {
			$data['displayPropertyOrder'] = serialize($data['displayPropertyOrder']);
			...
			if (!$data['hideIfSoldOut']) {
				$data['hideIfSoldOut'] = 0;
			}
      ...
      public function affixToProduct($productID) {
         //function is here because I think trying to use save for two fields would just ruin things.
         //ideally I would rewrite save to accept partial $data.  As it is it would reset the block to its defaults.
         $db = Loader::db();
         $db->execute("update btCoreCommerceProduct set inheritProductIDFromCurrentPage = ?, productID = ? where bID = ?", array(0, (int) $productID, (int) $this->bID));
      }
	}
