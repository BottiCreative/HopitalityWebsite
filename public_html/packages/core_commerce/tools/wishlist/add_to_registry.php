...
<div id="ccm-wishlist-dialog-content">
	<h2><?php  echo t('Add to gift registry')?></h2>
	<div class="ccm-message" style="display:none;"><?php  echo t('Item added to gift registry.')?></div>
	<form>
<?php 
$u = new User();
if($u->isLoggedIn()) {
	$wlh = Loader::helper('user_registries','core_commerce');
	...
	if(is_array($lists) && count($lists) == 1) { // only one wishlist
		...
		<?php  
			foreach($lists as $l) {
				echo '<option value="'.$wlh->getUpdateAction($l).'" '.($_SESSION['coreCommerceLastAccessedregistryID'] == $l->getCollectionID()?"selected=\"selected\"":"").'>'.
	...
	<?php  } ?>
	<br/>
	<div id="ccm-wishlist-manage-link"><a href="<?php  echo View::url('/gift_registries');?>"><?php  echo t('Create and manage gift registries')?></a></div>
<?php  } else {  // not logged in user message ?>
	<div class="ccm-message">
		<?php  echo t('You must be logged in to add items to a gift registry.') ?><br/>
